...
The bundled demonstration anatomy is de-identified, MRI-derived, and nonclinical. The connected MAT model is research-only: it has not been validated for this product, does not segment fibula or safety anatomy, and produced meshes still require identity, scale, orientation, topology, and surface-accuracy review. The tibial demo display mesh is not watertight. No encrypted institutional persistence layer or exact patient-mesh/BVH clearance backend is claimed. Browser storage is suitable only for a de-identified demonstration snapshot. Before any clinical validation, the safest next step is an independent radiology/engineering review of de-identified cases against approved reference segmentations, including landmark-based transform checks and surface-distance ground truth, before any surgical workflow evaluation.

## Hosted demonstration

The public website is deployed at [multilig-planner.org](https://multilig-planner.org/). It has two root-level Vite entries: `index.html` provides the educational overview of multiligament knee surgery, and `demo.html` hosts the current Viewer v2 planning application. Both pages share the same two-destination navigation and visual identity.

The static demo opens with the same de-identified MRI-derived femur/tibia display surfaces and sanitized default ligament plan on every fresh browser origin. Only geometry and the editable planning parameters are packaged. The build does not contain source MRI pixels, DICOM metadata, label maps, patient/study identifiers, local plan exports, inference reports, local paths, or nnUNet model files. MRI import and nnUNetv2 inference remain local-only through the loopback bridge described above; the public site does not upload clinical images or provide a clinical segmentation service. The bundled surfaces remain unreviewed, research/demo-only, and subject to the source study's consent, data-use agreement, and institutional policy.
